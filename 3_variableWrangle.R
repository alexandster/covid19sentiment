...
#RUCC
RUCC <- subset(read_xls("ruralurbancodes2013.xls"), select = c(FIPS, RUCC_2013))
...
vulner <- read.csv("SVI2018_US_COUNTY.csv") %>%
...
univ <- read.csv("Colleges_and_Universities.csv")
...
any(is.na(resp2))
write.csv(resp2, "cases_v3.csv", row.names = FALSE)
#----------------------------------------------------------------
